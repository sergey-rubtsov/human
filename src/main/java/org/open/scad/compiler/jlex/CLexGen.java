...
class CLexGen {
    ...
    CLexGen(String filename) throws java.io.IOException {
        new CLexGen(filename, "Yylex.java");
    }

    /***************************************************************
     Function: CLexGen
     **************************************************************/
    CLexGen(String path, String filename) throws java.io.IOException {
        /* Successful initialization flag. */
        m_init_flag = false;

        /* Open input stream. */
        m_instream = new java.io.FileReader(path + filename);
        if (null == m_instream) {
        ...
        m_outstream
                ...
                new java.io.FileWriter(path + filename.substring(0,1).toUpperCase()
                        + filename.substring(1).toLowerCase() + ".java.")));
        if (null == m_outstream) {
            System.out.println("Error: Unable to open output file " + path
                    + filename + ".java.");
            return;
        }
                    ...
                    case 'c':
                        if (0 == CUtility.charncmp(m_input.m_line,
                            ...
                            m_spec.m_implements_name =
                                    "org.open.scad.parser.runtime.Scanner".toCharArray();
                            m_spec.m_function_name =
                                    "next_token".toCharArray();
                            m_spec.m_type_name =
                                    "org.open.scad.parser.runtime.Symbol".toCharArray();
                            break;
                        }
    ...
    (
        ...
        if (false == m_spec.m_cup_compatible) {
            ...
            System.out.println("(Scanner implements "
                    + "org.open.scad.parser.runtime.Scanner.)");
        } else {
