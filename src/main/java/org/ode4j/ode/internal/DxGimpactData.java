...
public class DxGimpactData extends DxTriMeshData {
    ...
    private double[] m_Vertices;//const char* m_Vertices;
...
// 	}
    ...
    public void build(final float[] vertices,
                      final int[] indices) {
        this.build(Common.convertFloatsToDoubles(vertices), indices);
    }

    @Override
    public void build(double[] vertices, int[] indices) {
        dIASSERT(vertices != null);
        dIASSERT(indices != null);
        m_Vertices = vertices;
        m_Indices = indices;
...
//	void dGeomTriMeshDataSetBuffer(DTriMeshData g, ByteBuffer buf) {}
    ...
    public double getEdgeAngle(int triangle, int edge) {
        return (m_Angles != null ? m_Angles[triangle * 3 + edge] : Math.PI * 2);
    }
