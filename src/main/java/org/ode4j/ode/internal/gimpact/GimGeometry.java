...
public class GimGeometry extends GimMath {
    ...
    /// scalar times vector
    static final void VEC_SCALE_2(vec2f c, final double a, vec2f b) {
        (c).f[0] = (a) * (b).f[0];
        (c).f[1] = (a) * (b).f[1];
    }
    ...
    /// scalar times vector
    static final void VEC_SCALE(vec3f c, final double a, vec3f b) {
        (c).f[0] = (a) * (b).f[0];
        (c).f[1] = (a) * (b).f[1];
        (c).f[2] = (a) * (b).f[2];
    }

    static final void VEC_SCALE(vec3f c, final double a, vec4f b) {
        (c).f[0] = (a) * (b).f[0];
        (c).f[1] = (a) * (b).f[1];
        (c).f[2] = (a) * (b).f[2];
    }
    ...
    /// scalar times vector
    static final void VEC_SCALE_4(vec4f c, final double a, vec4f b) {
        (c).f[0] = (a) * (b).f[0];
        (c).f[1] = (a) * (b).f[1];
        (c).f[2] = (a) * (b).f[2];
        (c).f[3] = (a) * (b).f[3];
    ...
    static final void VEC_ACCUM_2(vec2f c, final double a, vec2f b) {
        (c).f[0] += (a) * (b).f[0];
        (c).f[1] += (a) * (b).f[1];
    }
    ...
    /// accumulate scaled vector
    static final void VEC_ACCUM(vec3f c, final double a, vec3f b) {
        (c).f[0] += (a) * (b).f[0];
        (c).f[1] += (a) * (b).f[1];
        (c).f[2] += (a) * (b).f[2];
    }
    ...
    /// accumulate scaled vector
    static final void VEC_ACCUM_4(vec4f c, final double a, vec4f b) {
        (c).f[0] += (a) * (b).f[0];
        (c).f[1] += (a) * (b).f[1];
        (c).f[2] += (a) * (b).f[2];
        (c).f[3] += (a) * (b).f[3];
    ...
    /// Vector dot product
    static final double VEC_DOT_2(vec2f a, vec2f b) {
        return ((a).f[0] * (b).f[0] + (a).f[1] * (b).f[1]);
    }


    /// Vector dot product
    static final double VEC_DOT(vec3f a, vec3f b) {
        return a.f[0] * b.f[0] + a.f[1] * b.f[1] + a.f[2] * b.f[2];
    }

    static final double VEC_DOT(vec4f a, vec3f b) {
        return a.f[0] * b.f[0] + a.f[1] * b.f[1] + a.f[2] * b.f[2];
    }

    static final double VEC_DOT(vec3f a, vec4f b) {
        return a.f[0] * b.f[0] + a.f[1] * b.f[1] + a.f[2] * b.f[2];
    }

    static final double VEC_DOT(vec4f a, vec4f b) {
        return a.f[0] * b.f[0] + a.f[1] * b.f[1] + a.f[2] * b.f[2];
    }

    static final double VEC_DOT(vec4f a, double[] b) {
        return a.f[0] * b[0] + a.f[1] * b[1] + a.f[2] * b[2];
    }

    /// Vector dot product
    static final double VEC_DOT_4(vec4f a, vec4f b) {
    ...
    static final void VEC_IMPACT_SQ(final RefFloat bsq, final vec3f direction,
        ...
        double _llel_ = VEC_DOT(direction, position);
        bsq.d = VEC_DOT(position, position) - _llel_ * _llel_;
    }

    static final double VEC_IMPACT_SQ(final vec3f direction, final vec3f position) {
        double _llel_ = VEC_DOT(direction, position);
    ...
    static final double VEC_IMPACT(final vec3f direction, final vec3f position) {
        double bsq = VEC_IMPACT_SQ(direction, position);
        bsq = GIM_SQRT(bsq);
        return bsq;
    }
    ...
    static final void VEC_LENGTH(final vec3f a, final RefFloat l) {
        double _pp = VEC_DOT(a, a);
        l.d = GIM_SQRT(_pp);
    }

    static final double VEC_LENGTH(final vec3f a) {
        double _pp = VEC_DOT(a, a);
        return GIM_SQRT(_pp);
    }
    ...
    /// Vector inv length
    static final double VEC_INV_LENGTH(final vec3f a) {
        double _pp = VEC_DOT(a, a);
        return GIM_INV_SQRT(_pp);
    }

    static final double VEC_INV_LENGTH(final vec4f a) {
        double _pp = VEC_DOT(a, a);
        return GIM_INV_SQRT(_pp);
    }
...
//	}
    ...
    static final void VEC_CONJUGATE_LENGTH(final vec3f a, final RefFloat l) {
        double _pp = 1.0f - a.f[0] * a.f[0] - a.f[1] * a.f[1] - a.f[2] * a.f[2];
        l.d = GIM_SQRT(_pp);
    }
    ...
    /// Set Vector size
    static final void VEC_RENORMALIZE(vec3f a, final double newlen) {
        double len = VEC_INV_LENGTH(a);
        if (len < G_REAL_INFINITY) {
            len *= newlen;
            a.f[0] *= len;
    ...
    static final void VEC_BLEND_AB(vec3f vr, final double sa, vec3f a, final double sb, vec3f b) {
...
//TZ	static final void VEC_BLEND(vec3f vr, final double a, vec3f b, final double s)	{ VEC_BLEND_AB(vr,1-s,a,sb,s); }
...
//TZ	static final void VEC_SET3(vec3f a, vec3f b,op, vec3f c) { a[0]=b[0] op c[0]; a[1]=b[1] op c[1]; a[2]=b[2] op c[2]; }
	...
	*/
    ...
    /*! matrix rotation  X */
    static final void ROTX_CS(mat4f m, final double cosine, final double sine) {
    ...
    /*! matrix rotation  Y */
    static final void ROTY_CS(mat4f m, final double cosine, final double sine) {
    ...
    /*! matrix rotation  Z */
    static final void ROTZ_CS(mat4f m, final double cosine, final double sine) {
...
*/
    ...
    static final void SCALE_MATRIX_2X2(mat2f b, final double s, mat2f a) {
    ...
    /*! multiply matrix by scalar */
    static final void SCALE_MATRIX_3X3(mat3f b, final double s, mat3f a) {
        b.f[M(0, 0)] = (s) * a.f[M(0, 0)];
        b.f[M(0, 1)] = (s) * a.f[M(0, 1)];
    ...
    /*! multiply matrix by scalar */
    static final void SCALE_MATRIX_4X4(mat4f b, final double s, mat4f a) {
        b.f[M(0, 0)] = (s) * a.f[M(0, 0)];
        b.f[M(0, 1)] = (s) * a.f[M(0, 1)];
        b.f[M(0, 2)] = (s) * a.f[M(0, 2)];
    ...
    /*! multiply matrix by scalar */
    static final void ACCUM_SCALE_MATRIX_2X2(mat2f b, final double s, mat2f a) {
        b.f[M(0, 0)] += (s) * a.f[M(0, 0)];
        b.f[M(0, 1)] += (s) * a.f[M(0, 1)];
    ...
    /*! multiply matrix by scalar */
    static final void ACCUM_SCALE_MATRIX_3X3(mat3f b, final double s, mat3f a) {
        b.f[M(0, 0)] += (s) * a.f[M(0, 0)];
        b.f[M(0, 1)] += (s) * a.f[M(0, 1)];
        b.f[M(0, 2)] += (s) * a.f[M(0, 2)];
    ...
    /*! multiply matrix by scalar */
    static final void ACCUM_SCALE_MATRIX_4X4(mat4f b, final double s, mat4f a) {
        b.f[M(0, 0)] += (s) * a.f[M(0, 0)];
        b.f[M(0, 1)] += (s) * a.f[M(0, 1)];
        b.f[M(0, 2)] += (s) * a.f[M(0, 2)];
        b.f[M(0, 3)] += (s) * a.f[M(0, 3)];
...
/*	static final void MAT_DOT_VEC_3X4(FloatArray p, mat4f m, FloatArray v)
     ...
     *
     */
    static final double COFACTOR_4X4_IJ(final mat4f m, final int i, final int j) {
        double fac = 0;
...
//	}
     ...
     */
    static final void SCALE_ADJOINT_2X2(mat2f a, final double s, mat2f m) {
     ...
     * Computes adjoint of matrix m, scales it by s, returning a
     */
    static final void SCALE_ADJOINT_3X3(mat3f a, final double s, mat3f m) {
     ...
     * Computes adjoint of matrix m, scales it by s, returning a
     */
    static final void SCALE_ADJOINT_4X4(mat4f a, final double s, mat4f m) {
        char _i_, _j_;
        for (_i_ = 0; _i_ < 4; _i_++) {
            for (_j_ = 0; _j_ < 4; _j_++) {
                a.f[M(_j_, _i_)] = COFACTOR_4X4_IJ(m, _i_, _j_);
    ...
    static final void AXIS_INTERSECT(final double min, final double max,
                                     final double a, final double d,
    ...
    static final void BOX_INTERSECTS_RAY(final aabb3f aabb, final vec3f vorigin,
                                         final vec3f vdir, final RefFloat tparam, final double tmax,
                                         final RefBoolean is_intersected) {
        //double _tfirst = 0.0f, _tlast = tmax;
    ...
    static final void AABB_PROJECTION_INTERVAL(final aabb3f aabb,
        ...
        double _center[] = { //TODO TZ optimize? use primitive variables? Store extent/centre in AABB?
        ...
        double _extend[] = {
                ...
                aabb.maxY - _center[1],
                aabb.maxZ - _center[2]};
        double _fOrigin = VEC_DOT(direction, _center);
        double _fMaximumExtent = _extend[0] * Math.abs(direction.f[0]) +
...
//	}
    ...
    static final double PLANEDIREPSILON = 0.0000001f;
    static final double PARALELENORMALS = 0.000001f;
    ...
    static final double DISTANCE_PLANE_POINT(final vec4f plane, final vec3f point) {
...
//			vec3f clipped2, RefInt intersection_type)
    ...
    static final void PLANE_MINOR_AXES(final vec4f plane,
        ...
        double A[] = {Math.abs(plane.f[0]), Math.abs(plane.f[1]), Math.abs(plane.f[2])};
        if (A[0] > A[1]) {
            if (A[0] > A[2]) {
...
//	    double det;
//		double e1e1 = VEC_DOT(dir1,dir1);
//		double e1e2 = VEC_DOT(dir1,dir2);
//		double e2e2 = VEC_DOT(dir2,dir2);
//		vec3f p1p2 = new vec3f();
//	    VEC_DIFF(p1p2,point1,point2);
//	    double p1p2e1 = VEC_DOT(p1p2,dir1);
//		double p1p2e2 = VEC_DOT(p1p2,dir2);
