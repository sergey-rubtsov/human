...
public class GimRadixSort {
    ...
    private static long RSORT_TOKEN_COMPARATOR(GIM_RSORT_TOKEN x, GIM_RSORT_TOKEN y) {
    ...
    static final GimRSortTokenComparator RSORT_TOKEN_COMPARATOR = new GimRSortTokenComparator() {
        @Override
        public long run(GIM_RSORT_TOKEN x, GIM_RSORT_TOKEN y) {
    ...
    //	#define D11_2(x)	(x >> 22 )
    private static int D11_0(long x) {
        return (int) (x & 0x7FFl);
    }

    private static int D11_1(long x) {
        return (int) (x >>> 11 & 0x7FFl);
    }

    private static int D11_2(long x) {
        return (int) (x >>> 22 & 0x3FFl);
    }
...
//                sorted[i++] = t;
    ...
    static final GimExchangeMacro GIM_DEF_EXCHANGE_MACRO = new GimExchangeMacro() {
        @Override
        public void run(GIM_RSORT_TOKEN[] array, int i, int j) {
            GIM_DEF_EXCHANGE_MACRO(array, i, j);
        }
    };

    //#define GIM_COMP_MACRO(x, y) ((GINT32)((x) - (y)))
    private static int GIM_COMP_MACRO(final int x, final int y) {
        return x - y;
    }
    //! @}
