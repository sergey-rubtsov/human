...
import static org.ode4j.ode.internal.gimpact.GimGeometry.CLOSEST_POINT_ON_SEGMENT;
...
import static org.ode4j.ode.internal.gimpact.GimGeometry.VEC_LENGTH;
...
//								GIM_TRIANGLE_CONTACT_DATA * contact_data)
    static boolean gim_triangle_sphere_collision(
            ...
            final vec3f center, final double radius,
            final GIM_TRIANGLE_CONTACT_DATA contact_data) {
        contact_data.m_point_count = 0;

        //Find Face plane distance
        double dis = DISTANCE_PLANE_POINT(tri.m_planes.m_planes[0], center);
...
//	{
    static void gim_trimesh_sphere_collision(final GimTrimesh trimesh, final vec3f center,
                                             final double radius, final GimDynArray<GimContact> contacts) {
        contacts.m_size = 0;
