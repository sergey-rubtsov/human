...
//			GIM_TRIANGLE_DATA tri2);
    ...
    static void RAY_TRIANGLE_INTERSECTION(vec3f vOrigin, vec3f vDir, vec3f vec1, vec3f vec2, vec3f vec3,
                                          vec4f tri_plane, vec3f pout, RefFloat u, RefFloat v, RefFloat tparam, final double tmax, RefBoolean does_intersect) {
...
//	}
    ...
    private boolean EDGE_EDGE_TEST(final vec3f V0, final vec3f U0, final vec3f U1,
                                   final double Ax, final double Ay, final int i0, final int i1) {
        double Bx = U0.f[i0] - U1.f[i0];
        double By = U0.f[i1] - U1.f[i1];
        double Cx = V0.f[i0] - U0.f[i0];
        double Cy = V0.f[i1] - U0.f[i1];
...
//	private boolean NEWCOMPUTE_INTERVALS(final double VV0, final double VV1, final double VV2,
...
//								GIM_TRIANGLE_DATA *tri2)
    int gim_triangle_triangle_overlap(
        ...
        double du0 = 0, du1 = 0, du2 = 0, dv0 = 0, dv1 = 0, dv2 = 0;
        vec3f D = new vec3f();//double D[3];
        double[] isect1 = new double[2], isect2 = new double[2];  //TODO optimize? TZ
        double du0du1 = 0, du0du2 = 0, dv0dv1 = 0, dv0dv2 = 0;
        short index;
        double vp0, vp1, vp2;
        double up0, up1, up2;
        double bb, cc, max;
        ...
        {
            final double VV0 = vp0, VV1 = vp1, VV2 = vp2;
            final double D0 = dv0, D1 = dv1, D2 = dv2;
            final double D0D1 = dv0dv1, D0D2 = dv0dv2;
            if (D0D1 > 0.0f) {
                /* here we know that D0D2<=0.0 */
                /* that is D0, D1 are on the same side, D2 on the other or on the plane */
                a = VV2;
        ...
        {
            final double VV0 = up0, VV1 = up1, VV2 = up2;
            final double D0 = du0, D1 = du1, D2 = du2;
            final double D0D1 = du0du1, D0D2 = du0du2;
            if (D0D1 > 0.0f) {
                /* here we know that D0D2<=0.0 */
                /* that is D0, D1 are on the same side, D2 on the other or on the plane */
                d = VV2;
