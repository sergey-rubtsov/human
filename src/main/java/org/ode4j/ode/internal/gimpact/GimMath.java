...
//	private float FR(int x) {return Float.intBitsToFloat(x);}
    ...
    protected static float MAX3(float a, float b, float c) {
        return MAX(a, MAX(b, c));
    }

    //#define MIN3(a,b,c) MIN(a,MIN(b,c))
    protected static float MIN3(float a, float b, float c) {
        return MIN(a, MIN(b, c));
    }

    //#define IS_ZERO(value) ((value) < G_EPSILON &&  (value) > -G_EPSILON)
    protected static boolean IS_ZERO(float a) {
        return a < G_EPSILON && a > -G_EPSILON;
    }
    ...
    //#define MAX3(a,b,c) MAX(a,MAX(b,c))
    protected static double MAX3(double a, double b, double c) {
        return MAX(a, MAX(b, c));
    }

    //#define MIN3(a,b,c) MIN(a,MIN(b,c))
    protected static double MIN3(double a, double b, double c) {
        return MIN(a, MIN(b, c));
    }
...
//	private boolean IS_POSITIVE(float a) { return a >= G_EPSILON; }
    ...
    protected static double CLAMP(double number, double minval, double maxval) {
        return number < minval ? minval : (number > maxval ? maxval : number);
    }
...
//	    return rn;
    ...
    //#define GIM_INV_SQRT(va,isva)\
    protected static final double GIM_INV_SQRT(final double va) {
        double isva;
        if ((va) <= 0.0000001f) {
            (isva) = G_REAL_INFINITY;
        } else {
            //TODO check this out TZ
            return 1. / Math.sqrt(va);
//	        double _x = (va) * 0.5f;
//	        GUINT32 _y = 0x5f3759df - ( IR(va) >> 1);
//	        (isva) = FR(_y);
//	        (isva) = (isva) * ( 1.5f - ( _x * (isva) * (isva) ) );
    ...
    //#define GIM_SQRT(va,sva)\
    protected static final double GIM_SQRT(final double va) {
//	    GIM_INV_SQRT(va,sva);
//	    (sva) = 1.0f/(sva);
        //return 1.0f/GIM_INV_SQRT(va);  //TODO TZ optimize like this:
        return va * GIM_INV_SQRT(va);  //TODO TZ
        //return (double) Math.sqrt(va);
    }


    //! Computes 1.0f / sqrtf(x). Comes from Quake3. See http://www.magic-software.com/3DGEDInvSqrt.html
    static double gim_inv_sqrt(final double f) {
        return GIM_INV_SQRT(f);
    }
	...
	\sa gim_inv_sqrt
	*/
    static double gim_sqrt(final double f) {
        return GIM_SQRT(f);
    }
