...
//			}
    ...
    private static void dxStepIsland_Stage0_Joints(dxStepperStage0JointsCallContext callContext) {
    ...
    static void dxStepIsland_Stage1(dxStepperStage1CallContext stage1CallContext) {
    ...
    static void dxStepIsland_Stage2a(dxStepperStage2CallContext stage2CallContext) {
...

        {
            ...
            while ((ji = ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_J, nj)) != nj) {
                ...
                for (int j = infom; j != 0; ) {
                    --j;
                    int index = Jinfo.getFindex(j);
                    if (index != -1) {
                        Jinfo.setFindex(j, index + ofsi);
                    }
                }
            }
        }
    ...
    static void dxStepIsland_Stage2b(dxStepperStage2CallContext stage2CallContext) {
        final DxStepperProcessingCallContext callContext = stage2CallContext.m_stepperCallContext;
        final dxStepperLocalContext localContext = stage2CallContext.m_localContext;
        dJointWithInfo1[] jointinfosA = localContext.m_jointinfosA;
...

        {
            ...
            while ((bi = ThrsafeIncrementIntUpToLimit(stage2CallContext.m_bi_rhs_tmp, nb)) != nb) {
                double[] tmp1currA = rhs_tmp;
                int tmp1currP = bi * 8;
                double[] invIrowA = invI;
                int invIrowP = bi * 12;
                DxBody b = bodyA[bodyP + bi];
                // dSetZero(tmp1curr, 8); -- not needed
                for (int j = 0; j < 3; ++j)
                    tmp1currA[tmp1currP + j] = b.facc.get(j) * b.invMass + b.lvel.get(j) * stepsizeRecip;
    ...
    static void dxStepIsland_Stage2c(dxStepperStage2CallContext stage2CallContext) {
        final dxStepperLocalContext localContext = stage2CallContext.m_localContext;
        dJointWithInfo1[] jointinfosA = localContext.m_jointinfosA;
        int jointinfosP = localContext.m_jointinfosOfs;
        int nj = localContext.m_nj;
    ...
    static void dxStepIsland_Stage3(dxStepperStage3CallContext stage3CallContext) {
