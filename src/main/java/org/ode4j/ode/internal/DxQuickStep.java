...
import static org.ode4j.ode.internal.CommonEnums.dSA__MIN;
import static org.ode4j.ode.internal.CommonEnums.dV3E__AXES_MIN;
    ...
    private static void compute_invM_JT(AtomicInteger mi_storage, final int m, final double[] J, final double[] iMJ,
        ...
        while ((mi_step = Atomics.ThrsafeIncrementIntUpToLimit(mi_storage, m_steps)) != m_steps) {
            ...
            while (true) {
                ...
                for (int j = 0; j != JVE__L_COUNT; j++)
                    iMJ[iMJ_ptr + IMJ__1L_MIN + j] = k1 * J[J_ptr + JME__J1L_MIN + j];
                ...
                if (b2 != -1) {
                    ...
                    for (int j = 0; j != JVE__L_COUNT; j++)
                        iMJ[iMJ_ptr + IMJ__2L_MIN + j] = k2 * J[J_ptr + JME__J2L_MIN + j];
...
//		int findex;
    ...
    private static void dxQuickStepIsland_Stage0_Bodies(dxQuickStepperStage0BodiesCallContext callContext) {
...
//	#endif
    ...
    private static void dxQuickStepIsland_Stage0_Joints(dxQuickStepperStage0JointsCallContext callContext) {
    ...
    private static void dxQuickStepIsland_Stage1(dxQuickStepperStage1CallContext stage1CallContext) {
    ...
    private static void dxQuickStepIsland_Stage2aSync(final dxQuickStepperStage2CallContext stage2CallContext, TaskGroup group) {
    ...
    private static void dxQuickStepIsland_Stage2bSync(final dxQuickStepperStage2CallContext stage2CallContext, TaskGroup group) {
        DxStepperProcessingCallContext callContext = stage2CallContext.m_stepperCallContext;
        int allowedThreads = callContext.m_stepperAllowedThreads();
    ...
    private static void dxQuickStepIsland_Stage2a(dxQuickStepperStage2CallContext stage2CallContext) {
        ...
        final double stepsizeRecip = dRecip(callContext.m_stepSize());
        {
            ...
            while ((ji = Atomics.ThrsafeIncrementIntUpToLimit(stage2CallContext.m_ji_J, nj)) != nj) {
                ...
                for (int jCurr = jRow; jCurr != jEnd; jCurr += JME__MAX) {
                    DMatrix.dSetZero(J, jCurr + JME__J1_MIN, JME__J1_COUNT);
                    J[jCurr + JME_RHS] = 0.0;
                    J[jCurr + JME_CFM] = worldCFM;
                    DMatrix.dSetZero(J, jCurr + JME__J2_MIN, JME__J2_COUNT);
    ...
    private static void dxQuickStepIsland_Stage2b(dxQuickStepperStage2CallContext stage2CallContext) {
        final DxStepperProcessingCallContext callContext = stage2CallContext.m_stepperCallContext;
        final dxQuickStepperLocalContext localContext = stage2CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage2c(dxQuickStepperStage2CallContext stage2CallContext) {
        final dxQuickStepperLocalContext localContext = stage2CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage3(dxQuickStepperStage3CallContext stage3CallContext) {
    ...
    private static void dxQuickStepIsland_Stage4a(dxQuickStepperStage4CallContext stage4CallContext) {
        ...
        while ((ji_step = Atomics.ThrsafeIncrementIntUpToLimit(stage4CallContext.m_ji_4a, nj_steps)) != nj_steps) {
            ...
            DMatrix.dSetZero(lambda, lambdacurr, lambdsnext - lambdacurr);
        }
    }

    private static int dxQuickStepIsland_Stage4LCP_IterationSync(final dxQuickStepperStage4CallContext stage4CallContext, TaskGroup group) {
    ...
    private static void dxQuickStepIsland_Stage4LCP_iMJComputation(dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_iMJSync(final dxQuickStepperStage4CallContext stage4CallContext, TaskGroup group) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_fcStart(final dxQuickStepperStage4CallContext stage4CallContext, TaskGroup group) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_MTfcComputation(dxQuickStepperStage4CallContext stage4CallContext) {
        dxQuickStepIsland_Stage4LCP_MTfcComputation_cold(stage4CallContext);
    }

    private static void dxQuickStepIsland_Stage4LCP_MTfcComputation_cold(dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        ...
        while ((bi_step = Atomics.ThrsafeIncrementIntUpToLimit(stage4CallContext.m_mi_fc, nb_steps)) != nb_steps) {
            ...
            DMatrix.dSetZero(fc, bi * CFE__MAX, bicnt * CFE__MAX);
        }
    }

    // TODO: not needed if we stick to allocating m_cforce on each call
    private static void dxQuickStepIsland_Stage4LCP_STfcComputation(dxQuickStepperStage4CallContext stage4CallContext) {
        double[] fc = stage4CallContext.m_cforce;
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        int nb = callContext.m_islandBodiesCount();
    ...
    private static void dxQuickStepIsland_Stage4LCP_AdComputation(dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
        ...
        while ((mi_step = Atomics.ThrsafeIncrementIntUpToLimit(stage4CallContext.m_mi_Ad, m_steps)) != m_steps) {
            ...
            while (true) {
                double sum = 0;
                for (int j = JVE__MIN; j != JVE__MAX; j++)
                    sum += iMJ[iMJ_ptr + j + IMJ__1_MIN] * J[j_ptr + j + JME__J1_MIN];
                int b2 = jb[mi * 2 + 1];
                if (b2 != -1) {
                    for (int j = JVE__MIN; j != JVE__MAX; j++)
                        sum += iMJ[iMJ_ptr + j + IMJ__2_MIN] * J[j_ptr + j + JME__J2_MIN];
                }
    ...
    private static void dxQuickStepIsland_Stage4LCP_ReorderPrep(dxQuickStepperStage4CallContext stage4CallContext) {
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
        int m = localContext.m_m;
    ...
    private static void dxQuickStepIsland_Stage4LCP_IterationStartSingleThread(final dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        DxWorld world = callContext.m_world();
        int num_iterations = world.qs.num_iterations;
        for (int iteration = 0; iteration < num_iterations; iteration++) {
    ...
    private static int dxQuickStepIsland_Stage4LCP_IterationStart(final dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_ConstraintsReordering(dxQuickStepperStage4CallContext stage4CallContext) {
    ...
    private static boolean dxQuickStepIsland_Stage4LCP_ConstraintsShuffling(dxQuickStepperStage4CallContext stage4CallContext, int iteration) {
    ...
    private static void dxQuickStepIsland_Stage4LCP_LinksArraysZeroing(dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_DependencyMapForNewOrderRebuilding(dxQuickStepperStage4CallContext stage4CallContext) {
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_DependencyMapFromSavedLevelsReconstruction(dxQuickStepperStage4CallContext stage4CallContext) {
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage4LCP_ConstraintsReorderingSync(final dxQuickStepperStage4CallContext stage4CallContext, TaskGroup group) {
        int stage4LCP_Iteration_allowedThreads = stage4CallContext.m_LCP_IterationAllowedThreads;
    ...
    private static void dxQuickStepIsland_Stage4LCP_MTIteration(final dxQuickStepperStage4CallContext stage4CallContext, final int initiallyKnownToBeCompletedLevel) {
    ...
    private static void dxQuickStepIsland_Stage4LCP_STIteration(dxQuickStepperStage4CallContext stage4CallContext) {
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
        int m = localContext.m_m;
        for (int i = 0; i != m; ++i) {
    ...
    private static void dxQuickStepIsland_Stage4LCP_IterationStep(dxQuickStepperStage4CallContext stage4CallContext, int i) {
    ...
    private static void dxQuickStepIsland_Stage4b(dxQuickStepperStage4CallContext stage4CallContext) {
        DxStepperProcessingCallContext callContext = stage4CallContext.m_stepperCallContext;
        dxQuickStepperLocalContext localContext = stage4CallContext.m_localContext;
    ...
    private static void dxQuickStepIsland_Stage5(dxQuickStepperStage5CallContext stage5CallContext) {
    ...
    private static void dxQuickStepIsland_Stage6a(dxQuickStepperStage6CallContext stage6CallContext) {
    ...
    private static void dxQuickStepIsland_Stage6aSync(final dxQuickStepperStage6CallContext stage6CallContext, TaskGroup group) {
        dxQuickStepIsland_Stage6_VelocityCheck(stage6CallContext);
    ...
    private static void dxQuickStepIsland_Stage6b(dxQuickStepperStage6CallContext stage6CallContext) {
        DxStepperProcessingCallContext callContext = stage6CallContext.m_stepperCallContext;
