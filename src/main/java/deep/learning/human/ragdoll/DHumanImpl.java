...
/**
 * Human rag doll configuration based on http://www.monsterden.net/software/Human-pyode-tutorial with tiny fixes.
 */
public class DHumanImpl implements DHuman {
    ...
    public DHumanBone[] getBones() {
        DHumanBone[] bones = new DHumanBone[LEFT_HAND + 1];
        bones[CHEST] = new DHumanBoneImpl(getRightChestPos(), getLeftChestPos(), getChestRadius());
        bones[BELLY] = new DHumanBoneImpl(getBellyTopPos(), getBellyBottomPos(), getBellyRadius());
        bones[PELVIS] = new DHumanBoneImpl(getRightPelvisPos(), getLeftPelvisPos(), getPelvisRadius());
        bones[HEAD] = new DHumanBoneImpl(getHeadTopPos(), getHeadBottomPos(), getHeadRadius());
        DVector3 rHipPos = getRightHipPos();
        DVector3 rKneePos = getRightKneePos();
        DVector3 rAnklePos = getRightAnklePos();
        bones[RIGHT_UPPER_LEG] = new DHumanBoneImpl(rHipPos, rKneePos, getUpperLegRadius());
        bones[RIGHT_LOWER_LEG] = new DHumanBoneImpl(rKneePos, rAnklePos, getLowerLegRadius());
        bones[RIGHT_FOOT] = new DHumanBoneImpl(getRightHeelPos(), getRightToesPos(), getFootRadius());
        DVector3 lHipPos = getLeftHipPos();
        DVector3 lKneePos = getLeftKneePos();
        DVector3 lAnklePos = getLeftAnklePos();
        bones[LEFT_UPPER_LEG] = new DHumanBoneImpl(lHipPos, lKneePos, getUpperLegRadius());
        bones[LEFT_LOWER_LEG] = new DHumanBoneImpl(lKneePos, lAnklePos, getLowerLegRadius());
        bones[LEFT_FOOT] = new DHumanBoneImpl(getLeftHeelPos(), getLeftToesPos(), getFootRadius());
        DVector3 rShoulderPos = getRightShoulderPos();
        DVector3 rElbowPos = getRightElbowPos();
        DVector3 rWristPos = getRightWristPos();
        bones[RIGHT_UPPER_ARM] = new DHumanBoneImpl(rShoulderPos, rElbowPos, getUpperArmRadius());
        bones[RIGHT_FORE_ARM] = new DHumanBoneImpl(rElbowPos, rWristPos, getForeArmRadius());
        bones[RIGHT_HAND] = new DHumanBoneImpl(rWristPos, getRightFingersPos(), getHandRadius());
        DVector3 lShoulderPos = getLeftShoulderPos();
        DVector3 lElbowPos = getLeftElbowPos();
        DVector3 lWristPos = getLeftWristPos();
        bones[LEFT_UPPER_ARM] = new DHumanBoneImpl(lShoulderPos, lElbowPos, getUpperArmRadius());
        bones[LEFT_FORE_ARM] = new DHumanBoneImpl(lElbowPos, lWristPos, getForeArmRadius());
        bones[LEFT_HAND] = new DHumanBoneImpl(lWristPos, getLeftFingersPos(), getHandRadius());
        return bones;
    }
    ...
    public DHumanJoint[] getJoints() {
        DHumanJoint[] joints = new DHumanJoint[LEFT_WRIST + 1];
        joints[MID_SPINE] = new DHumanJointImpl(JointType.FIXED, CHEST, BELLY, null, null, null, 0, 0, 0, 0);
        joints[LOW_SPINE] = new DHumanJointImpl(JointType.FIXED, BELLY, PELVIS, null, null, null, 0, 0, 0, 0);
        joints[NECK] = new DHumanJointImpl(JointType.CONSTRAINED_BALL, CHEST, HEAD, getNeckPos(), upAxis, upAxis, -Math.PI * 0.25,
                Math.PI * 0.25, -Math.PI * 0.3, Math.PI * 0.3);
        joints[RIGHT_HIP] = new DHumanJointImpl(JointType.UNIVERSAL, PELVIS, RIGHT_UPPER_LEG, getRightHipPos(), bkwdAxis, rightAxis,
                -Math.PI * 0.1, Math.PI * 0.3, -Math.PI * 0.15, Math.PI * 0.5);
        joints[RIGHT_KNEE] = new DHumanJointImpl(JointType.HINGE, RIGHT_UPPER_LEG, RIGHT_LOWER_LEG, getRightKneePos(), leftAxis, null,
                0.0, Math.PI * 0.75, 0.0, 0.0);
        joints[RIGHT_ANKLE] = new DHumanJointImpl(JointType.HINGE, RIGHT_LOWER_LEG, RIGHT_FOOT, getRightAnklePos(), rightAxis, null,
                -Math.PI * 0.1, Math.PI * 0.05, 0.0, 0.0);
        joints[LEFT_HIP] = new DHumanJointImpl(JointType.UNIVERSAL, PELVIS, LEFT_UPPER_LEG, getLeftHipPos(), fwdAxis, rightAxis,
                -Math.PI * 0.1, Math.PI * 0.3, -Math.PI * 0.15, Math.PI * 0.5);
        joints[LEFT_KNEE] = new DHumanJointImpl(JointType.HINGE, LEFT_UPPER_LEG, LEFT_LOWER_LEG, getLeftKneePos(), leftAxis, null, 0.0,
                Math.PI * 0.75, 0.0, 0.0);
        joints[LEFT_ANKLE] = new DHumanJointImpl(JointType.HINGE, LEFT_LOWER_LEG, LEFT_FOOT, getLeftAnklePos(), rightAxis, null,
                -Math.PI * 0.1, Math.PI * 0.05, 0.0, 0.0);

        DVector3 axis = new DVector3(-1.0, -1.0, 4.0);
        axis.normalize();
        joints[RIGHT_SHOULDER] = new DHumanJointImpl(JointType.CONSTRAINED_BALL, CHEST, RIGHT_UPPER_ARM, getRightShoulderPos(), axis,
                leftAxis, -Math.PI * 0.45, Math.PI * 0.45, -Math.PI * 0.25, Math.PI * 0.25);
        joints[RIGHT_ELBOW] = new DHumanJointImpl(JointType.HINGE, RIGHT_UPPER_ARM, RIGHT_FORE_ARM, getRightElbowPos(), downAxis, null,
                0.0, Math.PI * 0.6, 0.0, 0.0);
        joints[RIGHT_WRIST] = new DHumanJointImpl(JointType.HINGE, RIGHT_FORE_ARM, RIGHT_HAND, getRightWristPos(), fwdAxis, null,
                -Math.PI * 0.1, Math.PI * 0.2, 0.0, 0.0);
        axis = new DVector3(1.0, -1.0, 4.0);
        axis.normalize();
        joints[LEFT_SHOULDER] = new DHumanJointImpl(JointType.CONSTRAINED_BALL, CHEST, LEFT_UPPER_ARM, getLeftShoulderPos(), axis,
                rightAxis, -Math.PI * 0.45, Math.PI * 0.45, -Math.PI * 0.25, Math.PI * 0.25);
        joints[LEFT_ELBOW] = new DHumanJointImpl(JointType.HINGE, LEFT_UPPER_ARM, LEFT_FORE_ARM, getLeftElbowPos(), upAxis, null, 0.0,
                Math.PI * 0.6, 0.0, 0.0);
        joints[LEFT_WRIST] = new DHumanJointImpl(JointType.HINGE, LEFT_FORE_ARM, LEFT_HAND, getLeftWristPos(), bkwdAxis, null,
                -Math.PI * 0.1, Math.PI * 0.2, 0.0, 0.0);
        return joints;
    }

    private DVector3 getRightChestPos() {
    ...
    public DVector3 getBellyTopPos() {
        return scaleAndOffset(new DVector3(0.0, chestH, 0.0));
    }

    public DVector3 getBellyBottomPos() {
        return scaleAndOffset(new DVector3(0.0, hipH, 0.0));
    }
